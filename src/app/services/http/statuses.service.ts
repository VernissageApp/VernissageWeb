...
export class StatusesService {
    ...
    public async reblog(id: string): Promise<Status> {
        const event$ = this.httpClient.post<Status>(this.apiService + '/api/v1/statuses/' + id + '/reblog', new ReblogRequest(StatusVisibility.Public));
        return await firstValueFrom(event$);
    }

    public async unreblog(id: string): Promise<Status> {
        const event$ = this.httpClient.post<Status>(this.apiService + '/api/v1/statuses/' + id + '/unreblog', null);
        return await firstValueFrom(event$);
    }

    public async favourite(id: string): Promise<Status> {
        const event$ = this.httpClient.post<Status>(this.apiService + '/api/v1/statuses/' + id + '/favourite', null);
        return await firstValueFrom(event$);
    }

    public async unfavourite(id: string): Promise<Status> {
        const event$ = this.httpClient.post<Status>(this.apiService + '/api/v1/statuses/' + id + '/unfavourite', null);
        return await firstValueFrom(event$);
    }

    public async bookmark(id: string): Promise<Status> {
        const event$ = this.httpClient.post<Status>(this.apiService + '/api/v1/statuses/' + id + '/bookmark', null);
        return await firstValueFrom(event$);
    }

    public async unbookmark(id: string): Promise<Status> {
        const event$ = this.httpClient.post<Status>(this.apiService + '/api/v1/statuses/' + id + '/unbookmark', null);
        return await firstValueFrom(event$);
    }
}
